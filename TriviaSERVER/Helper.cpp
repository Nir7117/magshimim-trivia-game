...
int Helper::ByteToInt(std::vector<unsigned char> buffer)
{
	int result = ZERO;
	std::vector<unsigned char> len(buffer.begin() + 1, buffer.begin() + 5);
	result |= (len[ZERO] << SHIFT_24);
	result |= (len[ONE] << SHIFT_16);
	result |= (len[TWO] << SHIFT_8);
	result |= len[THREE];
	return result;
}

void Helper::send_Msg(SOCKET client_socket, std::vector<unsigned char> buffer)
{
	const unsigned char* buffer_msg = buffer.data();
	size_t size = buffer.size();
	// Send data
	send(client_socket, (const char*)buffer_msg, size, 0);
}